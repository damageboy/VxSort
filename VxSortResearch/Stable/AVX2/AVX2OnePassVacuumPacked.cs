...
{
    public static class AVX2OnePassVacuumPacked
    {
        public static unsafe void QuickSort<T>(T[] array) where T : unmanaged, IComparable<T>
        {
            ...
            fixed (T* p = &array[0]) {
                if (typeof(T) == typeof(int))
                    QuickSortInt32.Sort((int*) p, array.Length);
            }
        }

        class QuickSortInt32
        {
            ...
            var arraySpan = new ReadOnlySpan<int>(array, length);
                ...
                while (array <= right) {
                    ...
                    data = PermuteVar8x32(data, BytePermTables.GetBytePermutation(pBase, mask));
