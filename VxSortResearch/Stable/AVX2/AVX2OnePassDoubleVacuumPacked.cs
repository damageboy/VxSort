...
{
    public static class AVX2OnePassDoubleVacuumPacked
    {
        public static unsafe void QuickSort<T>(T[] array) where T : unmanaged, IComparable<T>
        {
            ...
            fixed (T* p = &array[0]) {
                if (typeof(T) == typeof(int)) {
                    QuickSortInt((int*) p, array.Length);
                }
            }
        }
        ...
        static unsafe int VectorizedPartition(int* array, int length)
        {
            ...
            var pBase = BitPermTables.BitPermTablePtr;
            //var magicMask = MagicMask;
            ...
            while (array <= right) {
                ...
                data = PermuteVar8x32(data, GetBitPermutation(pBase, mask));
...
shortcut:
        ...
        static unsafe void QuickSortInt(int* array, int length)
        {
            ...
            if(sep == length) {
                ...
                if (sep > 2) {
                    QuickSortInt(array, sep - 1);
                }
                if (sep + 1 < length) {
                    QuickSortInt(array + sep, length - sep);
                }
            }
        }
    }
