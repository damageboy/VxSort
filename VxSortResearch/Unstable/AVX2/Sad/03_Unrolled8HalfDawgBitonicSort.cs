...
namespace VxSortResearch.Unstable.AVX2.Sad
{
    ...
    public static class DoublePumpOverlinedUnroll8HalfDawgBitonicSort
    {
...
#pragma warning restore 649
            ...
            static uint _pdepMask;
            static uint _popCountMirror;
            // ReSharper restore FieldCanBeMadeReadOnly.Local

            static VxSortInt32()
            {
                _pdepMask       = 0x3C_3C_3C_3CU;
                _popCountMirror = 0x20_20_20_20U;
            ...
            internal static Vector256<uint> Transpose(Vector256<byte> shuffler, Vector256<uint> data)
            {
                var t1 = Shuffle(data.AsByte(), shuffler).AsUInt32();
                var t2 = Shuffle(ShiftRightLogical(data, 4).AsByte(), shuffler).AsUInt32();
                return Permute2x128(t1, t2, 0b_00_10_00_00);
            }
            ...
            internal int* Partition8VectorsInPlace(int* left, int* right, long hint)
            {
                ...
                *right = int.MaxValue;
                ...
                var pBaseByte = BytePermTables.BytePermTableAlignedPtr;
                var pBaseBit = BitWithInterleavedPopCountPermTablePtr;
                tmpRight -= N;
                ...
                RT0 = PermuteVar8x32(RT0, BytePermTables.GetBytePermutationAligned(pBaseByte, rtMask));
                LT0 = PermuteVar8x32(LT0, BytePermTables.GetBytePermutationAligned(pBaseByte, ltMask));
                ...
                var pdepMask = _pdepMask;
                var popCountMirror = _popCountMirror;
                ...
                while (readLeft < readRight) {
                    ...
                    // Generate an 4-element mask in one 32 bit integer
                    ...
                    var maskyMcMaskFace = (uint) MoveMask(CompareGreaterThan(L4, P).AsSingle());
                    maskyMcMaskFace |= (uint) MoveMask(CompareGreaterThan(L5, P).AsSingle()) << 8;
                    maskyMcMaskFace |= (uint) MoveMask(CompareGreaterThan(L6, P).AsSingle()) << 16;
                    maskyMcMaskFace |= (uint) MoveMask(CompareGreaterThan(L7, P).AsSingle()) << 24;

                    // Load 8 permutation vectors+precalculated pop-counts at once
                    // as early as possible
                    var permutations = GatherVector128(
                        pBaseBit,
                        ConvertToVector128Int32(CreateScalarUnsafe(maskyMcMaskFace).AsByte()),
                        sizeof(int));

                    // Now process "regularly" an additional 4 elements
                    // This way while we wait for the more efficient, yet slower gather operation
                    // We give the CPU enough real work while the load is chugging along
                    ...
                    L0 = PermuteVar8x32(L0, BytePermTables.GetBytePermutationAligned(pBaseByte, m0));
                    var pc = PopCount(m0) << 2;
                    Store(writeLeft,  L0);
                    Store(writeRight, L0);
                    writeRight = (int *) ((byte *) writeRight - pc);
                    writeLeft  = (int *) ((byte *) writeLeft + (8 << 2) - pc);

                    var m1 = (uint) MoveMask(CompareGreaterThan(L1, P).AsSingle());
                    L1 = PermuteVar8x32(L1, BytePermTables.GetBytePermutationAligned(pBaseByte, m1));
                    pc = PopCount(m1) << 2;
                    Store(writeLeft,  L1);
                    Store(writeRight, L1);
                    writeRight = (int *) ((byte*)  writeRight - pc);
                    ...
                    L2 = PermuteVar8x32(L2, BytePermTables.GetBytePermutationAligned(pBaseByte, m2));
                    pc = PopCount(m2) << 2;
                    Store(writeLeft,  L2);
                    Store(writeRight, L2);
                    writeRight = (int *) ((byte*) writeRight - pc);
                    ...
                    L3 = PermuteVar8x32(L3, BytePermTables.GetBytePermutationAligned(pBaseByte, m3));
                    pc = PopCount(m3) << 2;
                    Store(writeLeft,  L3);
                    Store(writeRight, L3);
                    writeRight = (int *) ((byte *) writeRight - pc);
                    ...
                    var halfTotalRight = PopCount(maskyMcMaskFace);
                    Trace($"Second ½ Unroll Permuted L|R: {HALF_SLACK_PER_SIDE_IN_ELEMENTS - halfTotalRight}|{halfTotalRight}");

                    var nextWriteRight = writeRight - halfTotalRight;
                    // Calculate the next side ASAP:
                    // At this point writeRight was advance 4 times for L0-L3
                    // And in one big chunk accounting for L4-L7. so it should be completely updated
                    nextSideIsRight = (byte*) nextWriteRight - (byte*) readRight <
                                      (2 * SLACK_PER_SIDE_IN_ELEMENTS - N) * sizeof(int);
                    Dbg($"Next side is: {(nextSideIsRight ? "RIGHT" : "LEFT")}");

                    // We've just loaded 4(!) permutation VECTORS (in packed form) into one Vector256
                    ...
                    var leftCounts = ParallelBitDeposit((uint) MoveMask(permutations.AsSByte()), pdepMask);
                    var rightCounts = popCountMirror - leftCounts;

                    var transposedPerms = Transpose(shuffler, Vector128.ToVector256Unsafe(permutations));
                    L4 = PermuteVar8x32(L4, transposedPerms.AsInt32());
                    L5 = PermuteVar8x32(L5, ShiftRightLogical(transposedPerms.AsInt32(), 08));
                    L6 = PermuteVar8x32(L6, ShiftRightLogical(transposedPerms.AsInt32(), 16));
                    L7 = PermuteVar8x32(L7, ShiftRightLogical(transposedPerms.AsInt32(), 24));

                    Store(writeLeft,  L4);
                    Store(writeRight, L4);
                    writeLeft  = (int*) ((byte*) writeLeft  + (leftCounts  & 0xFF));
                    writeRight = (int*) ((byte*) writeRight - (rightCounts & 0xFF));
                    Store(writeLeft,  L5);
                    Store(writeRight, L5);
                    writeLeft  = (int*) ((byte*) writeLeft  + ((leftCounts  >> 08) & 0xFF));
                    writeRight = (int*) ((byte*) writeRight - ((rightCounts >> 08) & 0xFF));
                    Store(writeLeft,  L6);
                    Store(writeRight, L6);
                    writeLeft  = (int*) ((byte*) writeLeft  + ((leftCounts  >> 16) & 0xFF));
                    writeRight = (int*) ((byte*) writeRight - ((rightCounts >> 16) & 0xFF));
                    Store(writeLeft, L7);
                    Store(writeRight, L7);
                    writeLeft = (int*) ((byte*) writeLeft  + ((leftCounts  >> 24) & 0xFF));
                    writeRight = nextWriteRight;
                }

                readRight += HALF_SLACK_PER_SIDE_IN_ELEMENTS;

                pBaseBit = BitPermTablePtr;
                while (readLeft < readRight) {
                    Stats.BumpScalarCompares();
                    ...
                    L0 = PermuteVar8x32(L0, GetBitPermutation(pBaseBit, m0));
                    L1 = PermuteVar8x32(L1, GetBitPermutation(pBaseBit, m1));
                    L2 = PermuteVar8x32(L2, GetBitPermutation(pBaseBit, m2));
                    L3 = PermuteVar8x32(L3, GetBitPermutation(pBaseBit, m3));
                ...
                while (readLeft <= readRight) {
                    ...
                    current = PermuteVar8x32(current, GetBitPermutation(pBaseBit, mask));
            ...
            static void Partition4VectorsToStack(int* p, int pivot, ref int* tmpLeft, ref int* tmpRight)
            {
                ...
                // Read-ahead 4 Vector256<int> from p
                ...
                var m0 = (uint) MoveMask(CompareGreaterThan(L0, P).AsSingle());
                var pc = PopCount(m0) << 2;
                L0 = PermuteVar8x32(L0, BytePermTables.GetBytePermutationAligned(pBase, m0));
                Store(tmpLeft, L0);
                Store(tmpRight, L0);
                tmpRight = (int*) ((byte*) tmpRight - pc);
                pc      = 32 - pc;
                tmpLeft = (int*) ((byte*) tmpLeft + pc);
                
                
                var m1 = (uint) MoveMask(CompareGreaterThan(L1, P).AsSingle());
                pc = PopCount(m1) << 2;
                L1 = PermuteVar8x32(L1, BytePermTables.GetBytePermutationAligned(pBase, m1));
                Store(tmpLeft, L1);
                Store(tmpRight, L1);
                tmpRight = (int*) ((byte*) tmpRight - pc);
                pc      = 32 - pc;
                ...
                var m2 = (uint) MoveMask(CompareGreaterThan(L2, P).AsSingle());
                pc = PopCount(m2) << 2;
                L2 = PermuteVar8x32(L2, BytePermTables.GetBytePermutationAligned(pBase, m2));
                Store(tmpLeft, L2);
                Store(tmpRight, L2);
                tmpRight = (int*) ((byte*) tmpRight - pc);
                pc      = 32 - pc;
                ...
                var m3 = (uint) MoveMask(CompareGreaterThan(L3, P).AsSingle());
                pc = PopCount(m3) << 2;
                L3 = PermuteVar8x32(L3, BytePermTables.GetBytePermutationAligned(pBase, m3));
                Store(tmpLeft, L3);
                Store(tmpRight, L3);
                tmpRight = (int*) ((byte*) tmpRight - pc);
                pc      = 32 - pc;
