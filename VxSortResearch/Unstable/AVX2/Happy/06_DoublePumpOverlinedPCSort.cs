...
namespace VxSortResearch.Unstable.AVX2.Happy
{
    ...
    public static class DoublePumpOverlinedPCSort
    {
        public static unsafe void Sort<T>(T[] array) where T : unmanaged, IComparable<T>
        {
            ...
            Stats.BumpSorts(nameof(DoublePumpOverlinedPCSort), array.Length);
            fixed (T* p = &array[0]) {
            ...
            internal void Sort(int* left, int* right, long hint)
            {
                ...
                if (length <= SMALL_SORT_THRESHOLD_ELEMENTS) {
                    ...
                    if (fakeLeft >= _startPtr) {
                        Dbg("Going for PCSort from {fakeLeft - left} -> {fakeLeft + nextLength - left}");
                ...
                var sep = Partition1VectorInPlace(left, right, hint);
