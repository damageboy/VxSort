...
namespace VxSortResearch.Unstable.AVX2.Happy
{
    ...
    public static class DoublePumpOverlinedUnroll4PCSort
    {
        public static unsafe void Sort<T>(T[] array) where T : unmanaged, IComparable<T>
        {
            ...
            Stats.BumpSorts(nameof(DoublePumpOverlinedUnroll4PCSort), array.Length);
            fixed (T* p = &array[0]) {
        ...
        const int SMALL_SORT_THRESHOLD_ELEMENTS = 64;
        ...
        internal unsafe struct VxSortInt32
        {
            ...
            const int PARTITION_TMP_SIZE_IN_ELEMENTS = (int) (2 * SLACK_PER_SIDE_IN_ELEMENTS + (2 * ALIGN) / sizeof(int));
            // We have to allocate 16 elements more given that we might
            // sort more data than our threshold due to alignment issues
            const int SMALL_SORT_TMP_SIZE_IN_ELEMENTS = SMALL_SORT_THRESHOLD_ELEMENTS * 2 + 16;

            const int TMP_SIZE_IN_ELEMENTS = SMALL_SORT_TMP_SIZE_IN_ELEMENTS > PARTITION_TMP_SIZE_IN_ELEMENTS
                ? SMALL_SORT_TMP_SIZE_IN_ELEMENTS
                : PARTITION_TMP_SIZE_IN_ELEMENTS;
...
#pragma warning restore 649
            ...
            public VxSortInt32(int* startPtr, int* endPtr) : this()
            {
                ...
                Dbg($"temp-size is {TMP_SIZE_IN_ELEMENTS}");
...
/*                    if (preAlignLeftOffset <= 0 && preAlignRightOffset > 0) {
                        Dbg(
                            $"Going for PCSort from 0x{(ulong) preAlignedRight:X16}({preAlignLeftOffset}) -> {preAlignedRight - preAlignedLeft}");
                        PositionCountingSort<int>.PCSort256Aligned(preAlignedLeft,
                            (int) (preAlignedRight - preAlignedLeft), _tempStart);
                    }else {
                        Dbg($"Going for Insertion Sort on [{left - _startPtr} -> {right - _startPtr - 1}]");
                        InsertionSort(left, right);
                    }
*/
                    var nextLength = (length & 0x7) > 0 ? (length + 8) & ~0x7 : length;
                    ...
                    if (fakeLeft >= _startPtr) {
                        ...
                        PositionCountingSort.Sort<int>(fakeLeft, nextLength, _tempStart);
                    }
                    else {
                        Dbg($"Going for Insertion Sort on [{left - _startPtr} -> {right - _startPtr - 1}]");
                        InsertionSort(left, right);
