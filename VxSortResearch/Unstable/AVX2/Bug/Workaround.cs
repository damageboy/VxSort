...
namespace VxSortResearch.Unstable.AVX2.Bug
{
    [LocalsInit(true)]
    public static class DoublePumpWorkaround
    {
        public static unsafe void Sort<T>(T[] array) where T : unmanaged, IComparable<T>
        {
            ...
            Stats.BumpSorts(nameof(DoublePumpNaive), array.Length);
            fixed (T* p = &array[0]) {
...
#pragma warning restore 649
            ...
            void PartitionBlock(int *dataPtr, Vector256<int> P, int* pBase, ref int* writeLeft, ref int* writeRight)
            {
                ...
                var m0 = (uint) MoveMask(CompareGreaterThan(dataVec, P).AsSingle());
                dataVec = PermuteVar8x32(dataVec, Int32PermTables.GetIntPermutation(pBase, m0));
                ...
                for (var t = left; t < boundary; t++)
                    if (!(*t <= pivot)) {
                        ...
                        Debug.Assert(*t <= pivot,
                            $"depth={_depth} boundary={boundary - left}, idx = {t - left} *t({*t}) <= pivot={pivot}");
                    }
