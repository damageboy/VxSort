...
namespace Bench.Utils
{
    ...
    public class DatatableJsonExporterAttribute : ExporterConfigBaseAttribute
    {
        ...
        public DatatableJsonExporterAttribute(bool indentJson = false, bool excludeMeasurements = false)
        ...
        public override void ExportToLog(Summary summary, ILogger logger)
        {
            ...
            var benchmarks = summary.Reports.Select(report => {
                ...
                var data = new Dictionary<string, object> {
                    ...
                    { "MeanDataTable", $"{report.ResultStatistics.Mean:0.0000} ({report.ResultStatistics.ConfidenceInterval.Lower:0.0000} - {report.ResultStatistics.ConfidenceInterval.Upper:0.0000})" },
                    { "TimePerNDataTable", $"{report.ResultStatistics.Mean/n:0.0000} ({report.ResultStatistics.ConfidenceInterval.Lower/n:0.0000} - {report.ResultStatistics.ConfidenceInterval.Upper/n:0.0000})" },
                ...
                var resultMeasurements = report.AllMeasurements.Where(m =>
                    m.IterationMode == IterationMode.Workload &&
                    m.IterationStage == IterationStage.Result).Select(m => m.Nanoseconds).ToArray();
